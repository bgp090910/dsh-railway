...
if [ ! -f "$MARKER" ]; then
  ...
  "$DSH" plugin --profile web add github:hi-wenw/dsh-telegram-channel || true
  ...
  "$DSH" plugin --profile web add github:kenz1117/dsh-engram || true

  # Autonomous heartbeat tasks
  "$DSH" plugin --profile web add github:Kanadego/dsh-heartbeat || true

  # Answer quality reviewer
  "$DSH" plugin --profile web add github:bycall/dsh-answer-reviewer || true

  # Prompt optimizer
  "$DSH" plugin --profile web add github:1321928757/dsh-prompt-polish || true
